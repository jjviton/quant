...
def main():
    ...
    start =dt.datetime.today() - dt.timedelta(days=3000)
    ...
    tickers2 = ['BKT.MC'] 
    tickers= ['FER.MC','COL.MC','NTGY.MC','SAB.MC','ACX.MC','PHM.MC','SAN.MC','MRL.MC','IBE.MC','TEF.MC','AMS.MC','VIS.MC','MTS.MC','MAP.MC','CLNX.MC','BBVA.MC','CABK.MC','MEL.MC','AENA.MC','BKT.MC','REE.MC','FDR.MC','ACS.MC','ITX.MC','ENG.MC','ANA.MC','ELE.MC','GRF.MC','IAG.MC','SGRE.MC']
...
def analisis(instrumento, start, end):
    ...
    # 0.- Lemos los datos 
    df =web.DataReader(instrumento, 'yahoo', start, end)  
    ...
    df_aux= df.iloc[-200:, [5]]   #5-> Adj Close
    ...
    if (coef_linear>0  and  coef_ema200_>0):
        ...
        if( df.iloc[indiceHoy_,price_] < ( 1* df.iloc[indiceHoy_,bb_dn_]) ):   #ajustar esto, solo necesito el dato de hoy
            señal =True
        else:
            señal = False
    ...
    if (señal == True):
        ...
        #Guardar las señales
        #df_sg = pd.DataFrame(columns=('estrategia','instrumento', 'fecha', 'aux1', 'aux2'))
        quant_j.saveSignal('RegresMedia_2', 'RegresionMedia b0', instrumento,dt.datetime.today() )
