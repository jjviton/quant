...
class TradingAI:
    ...
    def __init__(self, instrumento = 'IBE.MC'):   #BABA
        ##self.w = w
        ##self.h = h
        # init display
        ...
        #### FECHAS
        self.start =dt.datetime(2006,1,1)
    ...
    def featured_Data (self, dff):
        ...
        print(dff.head())
        ##quant_j.salvarExcelTOTEST(dff, 'iberdrola_01', nombreSheet="data")  
    ...
    def Observations (self, game, _stage):
        ...
        state[0] = game.df.loc[_stage,'Close']    
        state[1] = game.df.loc[_stage,'EMA_200']   
        state[2] = game.df.loc[_stage,'EMA_50']   
        state[3] = game.df.loc[_stage,'EMA_30']  
        ##state[4] = game.df.loc[_stage,'DeltaVol_EMA'] 
    ...
    def play_step(self, game, _stage, action):
        ...
        if (self.inside == True):       #estoy dentro del mercado comprado
            ...
            if(action[BUY]== True):     #me pide comprar
                ...
                if (game.df.loc[_stage,'Close'] > (1.0*self.valorCompra) ):
                    reward +=10
                    self.score +=1
                    self.profit +=1
                                        #elif (game.df.loc[_stage,'Futuro'] < (1.01*game.df.loc[_stage,'Close'])):
                else:                   #j#elif (game.df.loc[_stage,'Close'] <= (1.03*self.valorCompra)):
                    kk=self.inside
                    kk2=insidee
                    reward -=5
                    self.loss +=1
