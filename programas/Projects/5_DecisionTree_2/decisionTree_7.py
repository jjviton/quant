...
def analisisENTRADA(instrumento, startt, endd, df):    #analisis_v2
    ...
    df_aux30 =df.loc[:,['Kalman','Volume']]   # Copiamos una parte del dataframe
    ...
    df_aux30['target_cls'] = np.where(df_aux30.Return > 0, 1, 0)
    ...
    X2 = df_aux30.drop('target_rgs', axis=1)   #borro la columa que tiene los retornos en valor (regresion, no clasificacition)
    X2.drop('target_cls', axis=1, inplace=True)    
    X2.drop('Return', axis=1, inplace=True)  
