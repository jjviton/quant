...
####################################################### fucniones de prueba()
def handsOn(parametro11=0, parametro22=0):
    
    # Como leer la tabla de simbolos del SP500 desde wikipedia
    sp_url= 'https://en.wikipedia.org/wiki/List_of_S%26P_500_companies'
    sp=pd.read_html(sp_url,header=0)[0]
    print (sp.info())

    # Convierto el simbolo en Ticker    
    symbol=sp.iloc[0,0] 
    ticker = yf.Ticker(symbol)
    
    data= ticker.history(period= '44',interval='1m',actions=(True), auto_adjust=(True))
    ...
    """
    sp500_tickers[(sp[])]
    symbols = sp['Symbol'].values.tolist()
    print(symbols[:15])
    """
