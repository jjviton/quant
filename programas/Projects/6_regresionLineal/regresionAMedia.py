...
def estrategiaSALIDA(instrumento, startt, endd, df):
    ...
    if( PrecioHoy < (precioCompra_ - (stoploss_)) ):
        ...
        dfx2=StrategyClass.dfCartera
        if(TELEGRAM__):
            telegram_send("!! SL en la estrategia Regresión lineal v6\nStopLoss " +instrumento)
    ...
    ## TAKE_PROFIT
        ...
        dfx2=StrategyClass.dfCartera 
        if(TELEGRAM__):
            telegram_send("!! TP en la estrategia Regresión lineal v6\nTakeProfit " +instrumento)
...
def analisisENTRADA(instrumento, startt, endd, df):    #analisis_v2
    ...
    if (señal == True):
        ...
        if(TELEGRAM__):
            telegram_send("Señal en la estrategia 6 Regresión lineal v6\nMira (IN) " +instrumento)
...
if __name__ == '__main__':    
    ...
    if(TELEGRAM__):
        telegram_send("2.- Estrategia_6 Regresion lineal v6\n ")
    ...
    tickers = ['cnc'] 
    tickers4 = ['FER.MC','COL.MC','IBE.MC','NTGY.MC','SAB.MC','ACX.MC','PHM.MC','SAN.MC','MRL.MC','TEF.MC','AMS.MC','VIS.MC','MTS.MC','MAP.MC','CLNX.MC','BBVA.MC','CABK.MC','MEL.MC','AENA.MC','BKT.MC','REE.MC','FDR.MC','ACS.MC','ITX.MC','ENG.MC','ANA.MC','ELE.MC','GRF.MC','IAG.MC','SGRE.MC']
